...
def get_random_move(board):
    empty_cells = [(row, col) for row in range(3) for col in range(3) if is_valid_move(board, row, col)]
...
def tic_tac_toe():
    ...
    while difficulty not in ['easy', 'hard']:
        difficulty = input("Invalid choice. Choose difficulty (easy/hard): ").strip().lower()
